...
app.post('/api/shorturl', async (req, res) => {
  ...
  //ry initializing validMuch to not have error
  let validMuch;
  try { //asynch to get catch error
    ...
    if (validMuch.protocol !== 'http:' && validMuch.protocol !== 'https:') {
    ...
    let existUrl = await Url.findOne({ original_url: url });
    if (existUrl) {
    ...
    console.log("Generated shortr URL", newUrl.short_url);
    await newUrl.save();

    res.json({ 
      original_url: newUrl.original_url, 
      short_url: newUrl.short_url }); //give answer with original and short url
});
